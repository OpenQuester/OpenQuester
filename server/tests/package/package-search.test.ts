...
describe("Package Search API", () => {
  ...
  async function createMultiplePackages(options: {
    ...
    includeFinalRound?: boolean;
    authors?: "user1" | "user2" | "alternate" | ((index: number) => "user1" | "user2");
  }): Promise<number[]> {
    const {
      count,
      titlePrefix = "Package",
  ...
  describe("Text Search", () => {
    ...
    it("should find packages by partial title match (case-insensitive)", async () => {
      ...
      expect(result.data.every((p) => p.title.toLowerCase().includes("javascript"))).toBe(true);
    });
    ...
    it("should support wildcard searches with special characters", async () => {
      ...
      const res = await http.get("/v1/packages").query({ title: "C++", limit: 10, offset: 0 });
  ...
  describe("Filter by Fields", () => {
    it("should filter packages by language", async () => {
      ...
      const res = await http.get("/v1/packages").query({ language: "es", limit: 10, offset: 0 });
  ...
  describe("Filter by Tags", () => {
    it("should filter packages by single tag", async () => {
      ...
      const result: PaginatedResult<Omit<PackageDTO, "rounds">[]> = res.body;
      expect(result.data).toHaveLength(1);
      expect(result.data[0].tags?.some((t) => t.tag === "programming")).toBe(true);
    });
    ...
    it("should filter packages by multiple tags (any match)", async () => {
      ...
      expect(
        result.data.every((p) => p.tags?.some((t) => ["javascript", "python"].includes(t.tag)))
      ).toBe(true);
    });
    ...
    it("should handle comma-separated tags string", async () => {
      ...
      const res = await http.get("/v1/packages").query({ tags: "html,css", limit: 10, offset: 0 });
  ...
  describe("Filter by Stats (Rounds and Questions)", () => {
    it("should filter packages by minimum rounds count", async () => {
      ...
      const res = await http.get("/v1/packages").query({ minRounds: 2, limit: 10, offset: 0 });
    ...
    it("should filter packages by maximum rounds count", async () => {
      ...
      const res = await http.get("/v1/packages").query({ maxRounds: 1, limit: 10, offset: 0 });
    ...
    it("should filter packages by minimum questions count", async () => {
      ...
      const res = await http.get("/v1/packages").query({ minQuestions: 15, limit: 10, offset: 0 });
    ...
    it("should filter packages by maximum questions count", async () => {
      ...
      const res = await http.get("/v1/packages").query({ maxQuestions: 10, limit: 10, offset: 0 });
  ...
  describe("Combined Filters", () => {
    ...
    it("should combine author filter and tags", async () => {
      ...
      const res = await http.get("/v1/packages").query({
        authorId: user1.id,
        tags: ["programming"],
        limit: 10,
        offset: 0
  ...
  describe("Sorting and Pagination", () => {
    ...
    it("should handle empty results on pagination beyond available data", async () => {
      ...
      const res = await http.get("/v1/packages").query({ limit: 10, offset: 100 });
  ...
  describe("Validation and Edge Cases", () => {
    it("should require limit parameter", async () => {
      const res = await http.get("/v1/packages").query({ offset: 0 });

      expect(res.status).toBe(400);
    });

    it("should require offset parameter", async () => {
      const res = await http.get("/v1/packages").query({ limit: 10 });
    ...
    it("should reject invalid order value", async () => {
      const res = await http.get("/v1/packages").query({ order: "invalid", limit: 10, offset: 0 });

      expect(res.status).toBe(400);
    });

    it("should reject negative limit", async () => {
      const res = await http.get("/v1/packages").query({ limit: -1, offset: 0 });

      expect(res.status).toBe(400);
    });

    it("should reject negative offset", async () => {
      const res = await http.get("/v1/packages").query({ limit: 10, offset: -1 });
    ...
    it("should handle search with no packages in database", async () => {
      const res = await http.get("/v1/packages").query({ limit: 10, offset: 0 });
    ...
    it("should handle very long search strings", async () => {
      ...
      const res = await http.get("/v1/packages").query({ title: longString, limit: 10, offset: 0 });
  ...
  describe("Performance and Scalability", () => {
    it("should handle search across many packages efficiently", async () => {
      ...
      const res = await http.get("/v1/packages").query({ language: "en", limit: 20, offset: 0 });
    ...
    it("should handle complex filters on large dataset", async () => {
      ...
      await createMultiplePackages({
        ...
        descriptions: (i) => (i % 5 === 0 ? "special description" : "regular content"),
        languages: ["en", "ua", "es"],
        ageRestrictions: [AgeRestriction.NONE, AgeRestriction.A16, AgeRestriction.A18],
