...
describe("User Data Update on Game Join", () => {
  ...
  it("should update user meta when player rejoins game after profile change", () =>
    suite.scenario(async () => {
      ...
      const playerAfterLeave = gameAfterLeave.getPlayer(setup.playerUsers[0].id, {
      ...
      const gameDataOnRejoin: GameJoinOutputData = await utils.joinSpecificGameWithData(
  ...
  it("should use current user data for new player joining", () =>
    suite.scenario(async () => {
  ...
  it("should always update player meta data even when user data hasn't changed", () =>
    suite.scenario(async () => {
      // Create a user and game setup
      const setup = await utils.setupGameTestEnvironment(userRepo, app, 1, 0);
      ...
      expect(rejoiningPlayer).toBeDefined();
      expect(rejoiningPlayer!.meta.username).toBe(setup.playerUsers[0].username);
      expect(rejoiningPlayer!.status).toBe(PlayerGameStatus.IN_GAME);
