...
describe("Player Game Statistics Tests", () => {
  ...
  async function waitForPlayerKick(
    ...
    await utils.waitForEventMatching<PlayerKickBroadcastData>(
      socket,
  ...
  async function waitForPlayerRoleChange(
    ...
    await utils.waitForEventMatching<PlayerRoleChangeBroadcastData>(
      socket,
  ...
  describe("Skip Answer Handling", () => {
    it("should not count skipped questions as wrong answers", async () => {
      ...
      const simpleQuestionId = await utils.getQuestionIdByType(gameId, PackageQuestionType.SIMPLE);
