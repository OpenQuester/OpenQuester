...
describe("PlayerRestrictions", () => {
  ...
  it("should prevent restricted player from joining as PLAYER or SHOWMAN, allow only SPECTATOR", async () => {
    ...
    const joinEventPromise = utils.waitForEvent(setup.showmanSocket, SocketIOGameEvents.JOIN);
  ...
  it("should force-disconnect banned player after restriction and leave broadcasts", async () => {
    ...
    const disconnectPromise = utils.waitForEvent(bannedPlayerSocket, "disconnect");
  ...
  it("should successfully kick a player and emit LEAVE event", async () => {
    ...
    const kickEventPromise = utils.waitForEvent<PlayerKickBroadcastData>(
      setup.showmanSocket,
      SocketIOGameEvents.PLAYER_KICKED
    );

    const leaveEventPromise = utils.waitForEvent<GameLeaveEventPayload>(
      setup.showmanSocket,
      SocketIOGameEvents.LEAVE
    );

    const playerLeaveEventPromise = utils.waitForEvent<GameLeaveEventPayload>(
      playerSocket,
      SocketIOGameEvents.LEAVE
    );
