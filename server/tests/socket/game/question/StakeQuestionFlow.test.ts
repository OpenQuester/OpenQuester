...
describe("Stake Question Flow Tests", () => {
  ...
  describe("STAKE Question Selection", () => {
    ...
    describe("Sequential Bidding Process", () => {
      ...
      async function prepareScenario1(): Promise<void> {
        ...
        suite.currentScenario.actor(setup.playerSockets[0]).emit(SocketIOGameEvents.QUESTION_PICK, {
          questionId: stakeQuestionId
        });
      ...
      it("should allow pass bids after first bid", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const stakeQuestionPickedPromise = scenario.waitForEvent(
            showmanSocket,
            SocketIOGameEvents.STAKE_QUESTION_PICKED
          );

          // Player 0 (current turn player) starts stake question
          scenario.actor(setup.playerSockets[0]).emit(SocketIOGameEvents.QUESTION_PICK, {
            questionId: stakeQuestionId
          });
      ...
      it("should reject bid exceeding player score", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const errorPromise = scenario.waitForEvent(playerSockets[2], SocketIOEvents.ERROR);
      ...
      it("should reject bid when player score is lower than current highest bid", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const secondBidPromise = scenario.waitForEvent<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
      ...
      it("should restrict bidding to all-in or pass after someone bids all-in", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const playerTwoPassPromise = scenario.waitForEvent<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          scenario.actor(playerSockets[2]).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
          ...
          const playerZeroPassPromise = scenario.waitForEvent<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          scenario.actor(playerSockets[0]).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
      ...
      it("should reject bids from player who joined after stake bidding started", async () => {
        await suite.scenario(async (scenario) => {
          ...
          scenario.actor(lateJoinerSocket).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
            bidType: StakeBidType.NORMAL,
            bidAmount: 100
    ...
    describe("Bidding Completion and Winner Determination", () => {
      ...
      async function prepareScenario2(): Promise<void> {
        setup = await utils.setupGameTestEnvironment(userRepo, app, 3, 0);
        const { showmanSocket, gameId, playerUsers } = setup;
      ...
      it("should complete bidding and announce winner", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const showmanQuestionDataPromise = scenario.waitForEvent<GameQuestionDataEventPayload>(
            showmanSocket,
            SocketIOGameEvents.QUESTION_DATA
          );
          const playerQuestionDataPromise = scenario.waitForEvent<GameQuestionDataEventPayload>(
            playerSockets[0],
            SocketIOGameEvents.QUESTION_DATA
          );

          // Complete all bids: Player 0 -> 250, Player 1 -> 350, Player 2 -> pass, Player 0 -> pass (could outbid but chooses not to)
          const firstBidPromise = scenario.waitForEventMatching<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT,
          ...
          const secondBidPromise = scenario.waitForEventMatching<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT,
          ...
          const playerTwoPassPromise = scenario.waitForEventMatching<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT,
          ...
          const playerZeroPassPromise = scenario.waitForEventMatching<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT,
      ...
      it("should complete bidding on timer expiration with role-filtered question data", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const secondTimeoutBidPromise = scenario.waitForEvent<StakeBidSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          const winnerPromise = scenario.waitForEvent<StakeQuestionWinnerEventData>(
      ...
      it("should handle single highest bidder scenario", async () => {
        await suite.scenario(async (scenario) => {
          ...
          const firstPassPromise = scenario.waitForEvent(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
          ...
          const finalPassPromise = scenario.waitForEvent(
            showmanSocket,
            SocketIOGameEvents.STAKE_BID_SUBMIT
          );
          scenario.actor(playerSockets[2]).emit(SocketIOGameEvents.STAKE_BID_SUBMIT, {
