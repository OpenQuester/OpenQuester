  ...
  describe("Answer Submission Flow", () => {
    it("should allow players to submit answers and transition to reviewing phase", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const firstShowmanSubmitPromise =
          suite.currentScenario.waitForEventMatching<FinalAnswerSubmitOutputData>(
            showmanSocket,
        ...
        const submitEndPromise = scenario.waitForEvent<FinalSubmitEndEventData>(
          showmanSocket,
    ...
    it("should hide final answer text until submit end reveals all reviews", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const questionDataPromises = [
          ...
          scenario.waitForEvent(spectatorSocket, SocketIOGameEvents.FINAL_QUESTION_DATA)
        ];
        scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.FINAL_BID_SUBMIT, { bid: 600 });
        ...
        const firstSubmitPromises = [
          scenario.waitForEvent<FinalAnswerSubmitOutputData>(
            showmanSocket,
            SocketIOGameEvents.FINAL_ANSWER_SUBMIT
          ),
          scenario.waitForEvent<FinalAnswerSubmitOutputData>(
        ...
        const secondSubmitPromise = scenario.waitForEvent<FinalAnswerSubmitOutputData>(
          playerSockets[0],
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        const submitEndPromises = [
          scenario.waitForEvent<FinalSubmitEndEventData>(
            showmanSocket,
            SocketIOGameEvents.FINAL_SUBMIT_END
          ),
          scenario.waitForEvent<FinalSubmitEndEventData>(
    ...
    it("should broadcast timeout auto-losses and reveal all reviews after final answer timer expires", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const spectatorAutoLossesPromise =
          waitForPlayerBroadcasts<SocketIOFinalAutoLossEventPayload>(
            spectatorSocket,
            SocketIOGameEvents.FINAL_AUTO_LOSS,
            timeoutPlayerIds
          );
        ...
        for (const submitEnd of submitEndBroadcasts) {
          ...
          const timeoutReviews = submitEnd.allReviews?.filter(
            (review) =>
              review.playerId === playerUsers[1].id || review.playerId === playerUsers[2].id
          );
    ...
    it("should finish the game when all final answers time out as auto-loss", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const gameFinishedPromises = [
          scenario.waitForEvent<boolean>(showmanSocket, SocketIOGameEvents.GAME_FINISHED),
          scenario.waitForEvent<boolean>(playerSockets[0], SocketIOGameEvents.GAME_FINISHED),
          scenario.waitForEvent<boolean>(spectatorSocket, SocketIOGameEvents.GAME_FINISHED)
        ];
    ...
    it("should handle empty answers as auto-loss", async () => {
      await suite.scenario(async (scenario) => {
        ...
        // Submit bids to transition to answering phase
        const finalBidPromise = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_BID_SUBMIT
        );
        ...
        const finalBidPromise2 = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_BID_SUBMIT
        );
        scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.FINAL_BID_SUBMIT, { bid: 600 });
        ...
        const autoLossPromise =
          suite.currentScenario.waitForEventMatching<SocketIOFinalAutoLossEventPayload>(
            showmanSocket,
            SocketIOGameEvents.FINAL_AUTO_LOSS,
            (data) => data.playerId === playerUsers[0].id
          );
    ...
    it("should handle single player answering", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const phaseCompletePromise = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_PHASE_COMPLETE
        );
    ...
    it("should reject spectator final-round actions", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const spectatorEliminationErrorPromise = scenario.waitForEvent<Record<string, unknown>>(
          spectatorSocket,
          SocketIOEvents.ERROR
        );
        scenario.actor(spectatorSocket).emit(SocketIOGameEvents.THEME_ELIMINATE, {
        ...
        const spectatorBidErrorPromise = scenario.waitForEvent<Record<string, unknown>>(
          spectatorSocket,
          SocketIOEvents.ERROR
        );
        scenario.actor(spectatorSocket).emit(SocketIOGameEvents.FINAL_BID_SUBMIT, { bid: 100 });
        ...
        const spectatorAnswerErrorPromise = scenario.waitForEvent<Record<string, unknown>>(
          spectatorSocket,
          SocketIOEvents.ERROR
        );
        scenario.actor(spectatorSocket).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
    ...
    it("should accept max-length final answer and reject longer answers", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const maxLengthAnswerPromise = scenario.waitForEvent<FinalAnswerSubmitOutputData>(
          showmanSocket,
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        scenario.actor(playerSockets[0]).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
        ...
        const tooLongErrorPromise = scenario.waitForEvent<Record<string, unknown>>(
          playerSockets[1],
          SocketIOEvents.ERROR
        );
        scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
    ...
    it("should handle multiple players with mixed answer types", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const bidPromise2 = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_BID_SUBMIT
        );
        scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.FINAL_BID_SUBMIT, { bid: 600 });
        ...
        scenario.actor(playerSockets[2]).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
  ...
  describe("Review Flow", () => {
    it("should allow showman to review answers in any order and finish game", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const answerSubmitPromise = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        ...
        const answerSubmitPromise2 = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        ...
        const gameFinishedPromise = scenario.waitForEvent<boolean>(
          showmanSocket,
          SocketIOGameEvents.GAME_FINISHED
        );
    ...
    it("should handle mixed correct and incorrect reviews", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const phaseEndPromise = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_PHASE_COMPLETE
        );
        // Submit bids
        ...
        const answerPromise2 = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        scenario.actor(playerSockets[1]).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
        ...
        const answerPromise3 = scenario.waitForEvent(
          showmanSocket,
          SocketIOGameEvents.FINAL_ANSWER_SUBMIT
        );
        scenario.actor(playerSockets[2]).emit(SocketIOGameEvents.FINAL_ANSWER_SUBMIT, {
