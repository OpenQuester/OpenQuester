...
describe("Game Join Slot Allocation Edge Cases", () => {
  ...
  it("should correctly reassign slot when player leaves and rejoins during concurrent joins", async () => {
    ...
    const join1Promise = utils.waitForEvent(player1Socket, SocketIOGameEvents.GAME_DATA);
    const join2Promise = utils.waitForEvent(player2Socket, SocketIOGameEvents.GAME_DATA);
  ...
  describe("Target Slot Selection", () => {
    it("should allow joining with a specific target slot", async () => {
      ...
      const gameDataPromise = utils.waitForEvent(playerSocket, SocketIOGameEvents.GAME_DATA);
    ...
    it("should reject joining with an already occupied slot", async () => {
      ...
      const gameData1Promise = utils.waitForEvent(player1Socket, SocketIOGameEvents.GAME_DATA);
      player1Socket.emit(SocketIOGameEvents.JOIN, {
        gameId,
        role: PlayerRole.PLAYER,
        targetSlot: 1
      ...
      const errorPromise = utils.waitForEvent(player2Socket, SocketIOEvents.ERROR);
      player2Socket.emit(SocketIOGameEvents.JOIN, {
        gameId,
        role: PlayerRole.PLAYER,
    ...
    it("should auto-assign first available slot when targetSlot is not provided", async () => {
      ...
      const { socket: player2Socket, user: user2 } = await utils.createGameClient(app, userRepo);
      const gameData2Promise = utils.waitForEvent(player2Socket, SocketIOGameEvents.GAME_DATA);
      player2Socket.emit(SocketIOGameEvents.JOIN, {
        gameId,
        role: PlayerRole.PLAYER
    ...
    it("should ignore targetSlot for spectator role", async () => {
      ...
      const gameDataPromise = utils.waitForEvent(spectatorSocket, SocketIOGameEvents.GAME_DATA);
