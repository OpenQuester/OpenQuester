...
describe("Socket IO Self Role Change", () => {
  ...
  describe("Self Role Changes in Lobby", () => {
    it("should allow player to change to spectator", async () => {
      ...
      const roleChangeEventPromise = utils.waitForEvent<PlayerRoleChangeBroadcastData>(
    ...
    it("should allow showman to step down to player", async () => {
      ...
      const updatedPlayer = roleData.players.find((p: PlayerDTO) => p.meta.id === showmanUserId);
      expect(updatedPlayer).toBeDefined();
      ...
      const showmanExists = roleData.players.some((p: PlayerDTO) => p.role === PlayerRole.SHOWMAN);
  ...
  describe("Self Role Change Restrictions", () => {
    it("should prevent changing to same role", async () => {
      ...
      const errorPromise = utils.waitForEvent(setup.playerSockets[0], SocketIOEvents.ERROR, 2000);
  ...
  describe("Mid-Game Role Changes", () => {
    it("should allow self role changes during mid-game (not answering)", async () => {
      ...
      const gameStartPromise = utils.waitForEvent(showmanSocket, SocketIOGameEvents.START);
    ...
    it("should prevent self role changes during question answering", async () => {
      ...
      const errorPromise = utils.waitForEvent(playerSockets[0], SocketIOEvents.ERROR, 2000);
