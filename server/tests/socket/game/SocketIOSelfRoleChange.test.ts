  ...
  describe("Self Role Change Restrictions", () => {
    it("should prevent changing to same role", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const errorPromise = scenario.waitForEvent(
          setup.playerSockets[0],
  ...
  describe("Mid-Game Role Changes", () => {
    it("should allow self role changes during mid-game (not answering)", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const gameStartPromise = scenario.waitForEvent(showmanSocket, SocketIOGameEvents.START);
        ...
        scenario.actor(playerSockets[0]).emit(SocketIOGameEvents.PLAYER_ROLE_CHANGE, {
          newRole
        });
    ...
    it("should prevent self role changes during question answering", async () => {
      await suite.scenario(async (scenario) => {
        ...
        const errorPromise = scenario.waitForEvent(playerSockets[0], SocketIOEvents.ERROR, 2000);
