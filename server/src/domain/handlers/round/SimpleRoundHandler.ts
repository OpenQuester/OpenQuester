...
export class SimpleRoundHandler extends BaseRoundHandler {
  ...
  public async handleRoundProgression(
    game: Game,
    options: RoundProgressionOptions = { nextRound: null }
    ...
    if (forced) {
      return this.getProgressionState(game, options.nextRound);
    }
  ...
  private isAllQuestionsPlayed(game: Game): boolean {
    ...
    const { played, all } = GameQuestionMapper.getPlayedAndAllQuestions(game.gameState);
   ...
   */
  private async getProgressionState(
    game: Game,
  ...
  private initializeRoundPlayers(
    ...
    if (nextRound.type === PackageRoundType.FINAL) {
      const finalRoundData = FinalRoundStateManager.initializeFinalRoundData(game);

      finalRoundData.turnOrder = FinalRoundTurnManager.initializeTurnOrder(game);
