 ...
 *
 * Entry points:
 * - Showman marks answer as correct (AnswerResultUseCase)
...
export class AnsweringToShowingAnswerHandler extends BaseTransitionHandler {
  ...
  public canTransition(ctx: AnsweringToShowingAnswerCtx): boolean {
    ...
    if (trigger === TransitionTrigger.USER_ACTION || trigger === TransitionTrigger.PLAYER_LEFT) {
      ...
      const allExhausted =
        (answerType === AnswerResultType.WRONG || answerType === AnswerResultType.SKIP) &&
    ...
    return trigger === TransitionTrigger.TIMER_EXPIRED && game.willAllPlayersBeExhausted();
  }
  ...
  protected async mutate(ctx: AnsweringToShowingAnswerCtx): Promise<MutationResult> {
    ...
    const scoreResult = payload?.scoreResult ?? (isCorrect ? 0 : -currentQuestionPrice);
    ...
    if (currentQuestion && questionData) {
      ...
      GameQuestionMapper.setQuestionPlayed(game, question.id!, questionData.theme.id!);
    }
  ...
  protected async handleTimer(
    ...
    const { game } = ctx;
    const mutationData = mutationResult.data as AnsweringToShowingAnswerMutationData;
