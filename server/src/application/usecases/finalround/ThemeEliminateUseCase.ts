...
/**
 * Handles final round theme elimination.
 ...
 */
export class ThemeEliminateUseCase implements GameActionHandler<
  ThemeEliminateInputData,
  ThemeEliminateOutputData
> {
  constructor(
  ...
  public async execute(
    ...
    const finalRoundHandler = RoundHandlerFactory.create(
    ...
    if (result.isPhaseComplete) {
      const biddingMutationResult = BiddingInitializationLogic.processAutomaticBids(game);

      const biddingTransitionResult = await this.phaseTransitionRouter.tryTransition({
        game,
      ...
      mutations.push(
        ...DataMutationConverter.mutationFromTimerMutations(biddingTransitionResult?.timerMutations)
      );
