...
import { DataMutationConverter, type DataMutation } from "domain/types/action/DataMutation";
 ...
 *
 * For NORMAL questions, emits `question-data` immediately so existing clients
 * can preload media and report `media-downloaded`. The media gate opens later
 * through `media-download-status`.
 *
 * SECRET and STAKE questions emit events that are identical for all players.
 */
export class QuestionPickUseCase implements GameActionHandler<
...
> {
  ...
  public async execute(
    ...
    const transitionResources = this.transitionResourceService.fromQuestionWithTheme(questionData);
    ...
    let transitionResult = await this.phaseTransitionRouter.tryTransition<QuestionPickPayload>({
      game,
    ...
    const questionHasFiles = (questionData.question.questionFiles?.length ?? 0) > 0;

    if (initialTransitionResult.toPhase === GamePhase.MEDIA_DOWNLOADING && !questionHasFiles) {
    ...
    if (transitionResult.toPhase === GamePhase.STAKE_BIDDING && game.gameState.stakeQuestionData) {
    ...
    const questionDataMutations = this._buildNormalQuestionDataMutations(
      game,
      questionData.question,
      initialTransitionResult,
      questionHasFiles ? initialTransitionResult : transitionResult
    );
    ...
    return {
      ...
      mutations: [
        ...
        ...questionDataMutations,
        ...DataMutationConverter.mutationFromServiceBroadcasts(transitionResult.broadcasts, game.id)
      ],
      broadcastGame: game
    };
  }

  private _buildNormalQuestionDataMutations(
    game: Game,
    question: PackageQuestionDTO,
    initialTransitionResult: TransitionResult,
    timerTransitionResult: TransitionResult
  ): DataMutation[] {
    if (
      initialTransitionResult.toPhase !== GamePhase.MEDIA_DOWNLOADING ||
      !timerTransitionResult.timer ||
      !question.id
    ) {
      return [];
    }
    ...
    return [
      DataMutationConverter.gameBroadcastMutation(
        ...
        {
          data: question,
          timer: timerTransitionResult.timer,
  ...
  private async _autoAdvanceMediaDownloadWhenNoFiles(
    ...
    return {
      ...
      success: true,
      broadcasts: [...transitionResult.broadcasts, ...autoFinishTransition.broadcasts],
      timerMutations: [
