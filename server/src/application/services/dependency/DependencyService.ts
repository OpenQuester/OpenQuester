...
export class DependencyService {
   ...
   */
  public async getBulkFileUsage(filenames: string[]): Promise<Map<string, any[]>> {
    if (filenames.length === 0) {
      return new Map();
    }

    // Get all usage records for the filenames in a single query
    const allUsageRecords = await this.fileUsageService.getBulkUsageByFilenames(filenames);
